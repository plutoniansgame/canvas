...
import { PublicKey, ConfirmOptions, Transaction, SystemProgram, Keypair } from "@solana/web3.js";
...
import { CanvasModelForm, CanvasModelFormState } from "../components/CanvasModelForm";
...
const CanvasModelsPage = () => {
    ...
    const handleCanvasModelFormSubmit = async (data: CanvasModelFormState) => {
        ...
        const newModelAddress = await createCanvasModel(wallet.publicKey, data.name, data.collectionNFTAddress);
    ...
    const createCanvasModel = async (creator: PublicKey, canvasModelName: String, collectionNFTAddress: String): Promise<PublicKey> => {
        const tx = new Transaction();
        ...
        {canvasModels.map(model => (
            <Stack key={model.publicKey}>
                <Link href={`/canvas-model/${model.publicKey.toBase58()}`}>
                    <a>
                        <Typography>{model.account.name} -- {model.publicKey.toBase58()}</Typography>
                        <Typography>Slot Count: {model.account.slotCount}</Typography>
                    </a>
                </Link>
            </Stack>
        ))}
        {collectionNFTAddress && <Typography>{collectionNFTAddress}</Typography>}
        {!showCanvasModelForm && <Button onClick={handleCreateCollectionNFTClick}>Create Collection NFT</Button>}
        {!showCanvasModelForm && <Button onClick={() => setShowCanvasModelForm(true)}>New Canvas Model</Button>}
        {showCanvasModelForm && <CanvasModelForm onSubmit={handleCanvasModelFormSubmit} />}
    </Stack>
}
